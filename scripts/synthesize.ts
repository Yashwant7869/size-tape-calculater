...
function generate(n: number): SyntheticRow[] {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const trueChestCm = gender === "male"
      ? trueWaistCm * 1.32 + rand(-1.5, 1.5)
      : trueWaistCm * 1.34 + rand(-1.5, 1.5);
    const trueHipCm = gender === "male"
      ? trueWaistCm * 1.08 + rand(-1.5, 1.5)
      : trueWaistCm * 1.18 + rand(-1.5, 1.5);

    // Simulate photo at 720 × 1280 with the body taking ~70% of vertical extent.
    const imageW = 720, imageH = 1280;
    const scaleCmPerPx = heightCm / (imageH * 0.85);
    ...
    // For depth/width ratio 0.7: a = C / (π × 1.7), so diameter = 2a = 2C / (π × 1.7)
    const lateralWaistCm = trueWaistCm / (Math.PI * 1.7) * 2;
    ...
    const trueShoulderW = (lateralWaistCm * 1.55) / scaleCmPerPx;
    const trueHipWPx = (lateralWaistCm * 1.20) / scaleCmPerPx;
    const frontShoulderW = trueShoulderW + rand(-3, 3);
    const frontHipW = trueHipWPx + rand(-3, 3);
    // Top of head and bottom of feet
    const topMargin = 0.05 + rand(0, 0.03);
    const bottomMargin = 0.03 + rand(0, 0.03);
